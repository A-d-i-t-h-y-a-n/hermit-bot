...
async function setMessage(jid = null, tip = 'welcome', text = null) {
    ...
    if (Msg.length < 1) {
        return await GreetingsDB.create({ chat: jid, type: tip, message:text, enabled: true });
    } else {
        return await Msg[0].update({ chat: jid, type: tip, message:text, enabled: true });
    }
}
...
async function enableMessage(jid = null, tip = 'welcome') {
    ...
    if (Msg.length < 1) {
        return await GreetingsDB.create({ chat: jid, type: tip, enabled: true });
    } else {
        return await Msg[0].update({ chat: jid, type: tip, enabled: true });
    }
}
...
async function disableMessage(jid = null, tip = 'welcome') {
    ...
    if (Msg.length < 1) {
        return await GreetingsDB.create({ chat: jid, type: tip, enabled: false });
    } else {
        return await Msg[0].update({ chat: jid, type: tip, enabled: false });
    }
}
