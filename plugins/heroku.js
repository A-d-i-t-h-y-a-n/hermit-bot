...
const {Function,updatecheck,updatestart,formatTime,addCommand,getString} = require('../lib/');
...
await m.send('```No results found for this key```');}).catch(async (error) => {await m.send(`HEROKU : ${error.body.message}`);
});
});
