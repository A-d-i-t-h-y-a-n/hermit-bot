const { Function, isPublic, getString } = require("../lib/");
...
Function({pattern: 'removebg ?(.*)', fromMe: true, desc: Lang.REMOVEBG_DESC}, async (m, text, client) => {    
    ...
    var location = await client.downloadAndSaveMediaMessage(m.reply_message);
    var form = new FormData();
    form.append('image_file', fs.createReadStream(location));
    form.append('size', 'auto');
    var rbg = await got.stream.post('https://api.remove.bg/v1.0/removebg', {body: form, headers: {'X-Api-Key': Config.RBG_API_KEY}}); 
    await pipeline(rbg, fs.createWriteStream('rbg.png'));
    await client.sendMessage(m.jid, { image: fs.readFileSync('rbg.png') });
    await client.sendMessage(m.jid, { delete: load.key });
});
