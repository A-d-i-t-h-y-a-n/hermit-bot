        ...
        if (message.jid.includes('@g.us') && (message.mention !== false && message.mention.length !== 0)) {
            message.mention.map(async (jid) => {
                if (message.client.user.jid.split('@')[0] === jid.split('@')[0]) {
                    ...
                    (AFK.lastseen !== 0 ? '\n*' + Lang.LAST_SEEN + ':* ```' + secondsToHms(Math.round((new Date()).getTime() / 1000) - AFK.lastseen) + ' önce```' : ''), {quoted: message.data});            
                }
            })
...
Function({on: 'text', fromMe: true}, async (message, match) => {
...
Function({pattern: 'afk ?(.*)', fromMe: true, desc: Lang.AFK_DESC}, async (message, match) => {     
    if (!AFK.isAfk) {
